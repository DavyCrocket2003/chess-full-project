...
export default function StaticChessBoard({pieces, player1Id, player2Id}) {
  ...
  useEffect(() => {
    let whiteOnBottom = (onBottom==='regular' ? 'white' : 'black')
    ...
    <div className='useless'>
      {/* <DndContext modifiers={[snapCenterToCursor]} onDragEnd={handleDragEnd} onDragStart={handleDragStart}> */}
        {squareKeys && <div className="grid" >
          <Label userId={onBottom==="regular" ? player2Id : player1Id} />
            {squareKeys.map((key) => (    
              <StaticSquare key={key} id={key} piece={pieces[key]} bc={(+key[0] + +key[1]) % 2 ? whiteColor : blackColor}/>
            ))}
          <Label userId={onBottom==="regular" ? player1Id : player2Id}/>
        </div>}
      {/* </DndContext> */}
    </div>
  );
